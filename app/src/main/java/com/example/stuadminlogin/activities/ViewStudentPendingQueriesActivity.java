...
public class ViewStudentPendingQueriesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getSupportActionBar() != null) {
            getSupportActionBar().setTitle("My Pending Queries"); // Set your desired title
    ...
    private void loadPendingQueries() {
        ...
        try (SQLiteDatabase db = dbHelper.getReadableDatabase();
             Cursor cursor = db.rawQuery("SELECT query_text, generated_at FROM queries WHERE student_id = ? AND response_status = 'Pending' ORDER BY generated_at DESC", new String[]{String.valueOf(studentId)})) { // Added ORDER BY
            ...
            if (cursor.getCount() == 0) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    query.setResponseStatus("Pending"); // Manually set status for consistency if Query model needs it
                    pendingQueries.add(query);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            Toast.makeText(this, "Error loading pending queries: " + e.getMessage(), Toast.LENGTH_LONG).show();
        ...
        if (adapter == null) {
            ...
            adapter.notifyDataSetChanged(); // Notify if data changes (e.g., on resume if you implement that)
        }
    }
}