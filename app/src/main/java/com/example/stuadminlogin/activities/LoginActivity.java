...
public class LoginActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loginButton.setOnClickListener(v -> {
            ...
            // Using getWritableDatabase to update last_login
            ...
            } else if (selectedId == R.id.radio_parent) { // Handle Parent Login
                try (Cursor cursorParent = db.rawQuery("SELECT * FROM parents WHERE email=? AND password=?", new String[]{user, pass})) {
                    if (cursorParent != null && cursorParent.moveToFirst()) {
                        int parentId = cursorParent.getInt(cursorParent.getColumnIndexOrThrow("parent_id"));
                        db.execSQL("UPDATE parents SET last_login=? WHERE parent_id=?", new Object[]{currentDateTime, parentId});
                        sharedPreferences.edit().putInt("parent_id", parentId).apply();
                        Intent intent = new Intent(LoginActivity.this, ParentDashboardActivity.class);
                        intent.putExtra("parent_id", parentId);
                        startActivity(intent);
                        finish();
                    } else {
                        Toast.makeText(this, "Invalid Parent credentials", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    Log.e("LoginError", "Parent login failed", e);
                    Toast.makeText(this, "Parent login error!", Toast.LENGTH_SHORT).show();
                }
            }
            db.close();
        });
